...
class UseCaseModels:
    ...
    def generate_dot_code(self):
        for key, value in self.use.items():
            allowed_concepts = value["Concepts"]
            allowed_entities = value["Entities"]
            dot = setup_digraph()
            entities = []
            for key1, item in self.entities.items():
                ...
                table += "</table>>"
                if item["Entity"] in allowed_entities:
                ...
                for concept in self.entities[entity]["Concept"]:
                    if self.concepts.get(concept, False) and self.concepts[concept]["Relationship"]:
                        for rel in self.concepts[concept]["Relationship"]:
                            if rel["Object"] in self.conceptOfEntity and rel["Object"] in allowed_concepts:
                                dot.edge(entity, self.conceptOfEntity[rel["Object"]], rel["Relationship"])
                            else:
                                if rel["Object"] in node_concept and rel["Object"] in allowed_concepts:
                                    dot.edge(entity, f'node{strip_savvi(rel["Object"])}', rel["Relationship"])
                                elif rel["Object"] in allowed_concepts:
            ...
            # dot.graph_attr["label"] = key
            # dot.graph_attr["labelloc"] = "t"
            print(dot.source)
            dot.format = "svg"
            dot.render(os.path.join(os.path.dirname(__file__), f'graph_output/use_cases/{key}.gv'), view=False)
            dot.format = "png"
            dot.render(os.path.join(os.path.dirname(__file__), f'graph_output/use_cases/{key}.gv'), view=False)
